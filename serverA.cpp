...
void loadData(std::string inFileName, std::map<char, std::vector<std::vector<int>>>& data);
void printData(std::map<char, std::vector<std::vector<int>>> data);
...
void loadData(std::string inFileName, std::map<char, std::vector<std::vector<int>>>& data)
{
    ...
    while (getline(infile, line))
    {
        if (isupper(line.at(0))) // load ID
        {
            ...
            std::vector<int> prop_trans_nodenum(3);
        ...
        {
            data[previousMapID][0].push_back(std::stoi(line)); // prop_speed
            getline(infile, line);
            data[previousMapID][0].push_back(std::stoi(line)); // trans_speed
            continue;
        }
        std::vector<int> start_end_long(3);
        startNodeIndex = line.find(" ");
        start_end_long[0] = std::stoi(line.substr(0, startNodeIndex));
        nodeSet.insert(std::stoi(line.substr(0, startNodeIndex))); // count node
        endNodeIndex = line.find(" ", startNodeIndex + 1);
        start_end_long[1] = std::stoi(line.substr(startNodeIndex + 1, endNodeIndex));
...
void printData(std::map<char, std::vector<std::vector<int>>> data)
{
    ...
    {
        printf("%c\t%d\t\t%d\n", it->first, it->second[0][2], it->second.size() - 1);
    }
}
